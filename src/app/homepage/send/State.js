...
const [sendTradeToken,statusTradeToken]=WriteFunction(Contracts().tradeTokenContract,'startTrade',' starting a trade with a state')
...
const { toast } = useToast()
  ...
  async function onSubmitKey(values){
    ...
    <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel className="text-[18px] font-bold mt-8">Amount to send</FormLabel>
            <FormControl>
              <Input placeholder="PTAmount" onChangeCapture={e => setCurrencyValue(e.currentTarget.value)}{...field} />
              
            </FormControl>
            <FormDescription>Insert PT amount to send to the State above </FormDescription>
            <FormMessage />
          </FormItem>
        )}/>
        <GetCurrency ptAmount={currencyValue}/>
        <Button type="submit" className="bg bg-red-500">Send</Button>
        </form>
      </Form>
        ):<p className="text-l text-red-600 font-semibold mb-4 pt-4">You cannot trade with yourself</p>}
<div>
